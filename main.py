...
def main():
    ...
    check_file_integrity(files, read_files)

    controller.delete_file(files[0])
    controller.print_data()
    print(controller.read_all_data())

    try:
        check_file_integrity(files, controller.read_all_files())
...
def check_file_integrity(orig, read):
    if orig != read:
